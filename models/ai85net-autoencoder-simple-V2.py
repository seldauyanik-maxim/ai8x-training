...
class AutoEncoderSimpleV2(nn.Module):
    """
    Auto Encoder Model
    """
    def __init__(self,
...
class Decoder_FCNN(nn.Module):
    ...
    def __init__(self, num_features=128, **kwargs):
        ...
        self.decode_ConvTranspose1d1 = ai8x.FusedConvTranspose1dReLU(
            in_channels=self.in_chanels_for_the_first_layer,
            out_channels=16, kernel_size=3, stride=2,
            bias=True, padding=1)

        self.decode_ConvTranspose1d2 = ai8x.FusedConvTranspose1dReLU(
            in_channels=16, out_channels=32, kernel_size=3, stride=2, bias=True, padding=1)
        
        self.decode_Conv1d1 = ai8x.FusedConv1dReLU(
            in_channels=32, out_channels=32, kernel_size=3, padding=1, bias=True)

        self.decode_ConvTranspose1d3 = ai8x.FusedConvTranspose1dReLU(
            in_channels=32, out_channels=32, kernel_size=3, stride=2, bias=True, padding=1)

        self.decode_ConvTranspose1d4 = ai8x.FusedConvTranspose1dReLU(
            in_channels=32, out_channels=32, kernel_size=3, stride=2, padding=1, bias=True)
        
        self.decode_ConvTranspose1d5 = ai8x.FusedConvTranspose1dReLU(
            in_channels=32, out_channels=32, kernel_size=3, stride=2, padding=1, bias=True)
        
        self.decode_Conv1d2 = ai8x.FusedConv1dReLU(
            in_channels=32, out_channels=32, kernel_size=5, padding=2, bias=True)
        
        self.decode_ConvTranspose1d6 = ai8x.FusedConvTranspose1dReLU(
            in_channels=32, out_channels=32, kernel_size=3, stride=2, padding=1, bias=True)
        
        self.decode_ConvTranspose1d7 = ai8x.ConvTranspose1d(
            in_channels=32, out_channels=3, kernel_size=3, stride=2, padding=1, bias=True)
    ...
    def decode(self, x):
        ...
        x=self.decode_ConvTranspose1d1(x)
        x=self.decode_ConvTranspose1d2(x)
        x=self.decode_Conv1d1(x)
        x=self.decode_ConvTranspose1d3(x)
        x=self.decode_ConvTranspose1d4(x)
        x=self.decode_ConvTranspose1d5(x)
        x=self.decode_Conv1d2(x)
        x=self.decode_ConvTranspose1d6(x)
        x=self.decode_ConvTranspose1d7(x)
...
def ai85autoencodersimplev2(pretrained=False, **kwargs):
    """
    Returns an Auto Encoder Model
    """
    assert not pretrained
    return AutoEncoderSimpleV2(**kwargs)


def ai85autoencodersimplev2_16feats(pretrained=False, **kwargs):
    """
    Returns an Auto Encoder Model
    """
    assert not pretrained
    return AutoEncoderSimpleV2(num_features=16, **kwargs)


models = [
    {
        'name': 'ai85autoencodersimplev2',
        'min_input': 1,
        'dim': 1,
    },
    {
        'name': 'ai85autoencodersimplev2_16feats',
        'min_input': 1,
        'dim': 1,
    }
]
